<div align="center">

# 🌐 **MD NISHAN – Personal Website**

### Welcome to my personal website project  
Built with **HTML, CSS & JavaScript** and hosted on **Render** 🚀  

<br>

[![Website Status](https://img.shields.io/badge/Live%20Website-Online-brightgreen?style=for-the-badge&logo=google-chrome)](https://personal-iw3l.onrender.com/)
[![HTML](https://img.shields.io/badge/HTML-5-orange?style=for-the-badge&logo=html5)](#)
[![CSS](https://img.shields.io/badge/CSS-3-blue?style=for-the-badge&logo=css3)](#)

</div>

---

## 🧑‍💻 **About This Project**

This repository contains the source code of my **personal information website**, where I showcase:

- 👤 **About Me Section**
- 🖼️ **Photo Gallery**
- 🔗 **All my Social Media Links**
- 🌓 **Dark/Light Mode Toggle**
- ✨ **Animated Intro Screen**
- ⏳ **Loading Animation**
- 📩 **Fully Working Contact Form**
- 📊 **Google Analytics Tracking**
- 🎉 **Message Sent Animation Page**

---

## 🌟 **Live Demo**

Click below to visit my website:

👉 **https://personal-iw3l.onrender.com/**

---

## 📁 **Project Pages**

### 🏠 **Home Page (index.html)**
- About me  
- Profile image  
- Social media list  
- Visitor counter  
- Contact form with auto-email reply  
- Dark/Light mode  
- Intro + loading animation  

### 🖼️ **Gallery Page (gallery.html)**
- Responsive photo grid  
- Zoom-on-hover animation  
- Intro animation  
- Fully mobile-friendly  

### ✅ **Success Page (success.html)**
- Animated checkmark  
- Clean message  
- Auto redirect button  

---

## 🔧 **Tech Stack**

```txt
Frontend   : HTML, CSS, JavaScript  
Hosting    : Render  
Forms      : FormSubmit (auto email)  
Analytics  : Google Analytics
